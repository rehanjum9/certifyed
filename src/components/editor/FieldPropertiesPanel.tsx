...
import type { EditorField, TextAlign } from "@/lib/editor/types";
...
export function FieldPropertiesPanel({
  ...
  const keyError = fieldKeyError(field.field_key, otherFieldKeys);
  ...
  return (
    <div className="flex h-full flex-col overflow-y-auto p-4">
      <div className="flex flex-col gap-5">
        ...
        <section className="flex flex-col gap-3 border-t border-slate-200 pt-4">
          <h3 className="text-xs font-semibold uppercase tracking-wide text-slate-400">Auto-fit</h3>

          <label className="flex items-center gap-2 text-sm text-slate-700">
            <input
              type="checkbox"
              checked={field.auto_fit_text}
              onChange={(e) => {
                const auto_fit_text = e.target.checked;
                if (auto_fit_text && (field.min_font_size == null || field.max_font_size == null)) {
                  onChangeField({
                    auto_fit_text,
            ...
            <>
              ...
              {overflowing && (
                <p className="rounded-md bg-red-50 px-3 py-2 text-xs text-red-700">
                  The preview text still doesn&apos;t fit at the minimum font size.
                </p>
              )}
            </>
          )}
